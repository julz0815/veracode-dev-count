 ...
 */
async function execGitWithEnv(command, env) {
...
class GitService {
     ...
     */
    injectTokenIntoUrl(url, token, isGitLabCI = false, isUsingCIJobToken = false) {
     ...
     */
    async branchExistsLocally(branchName) {
        try {
     ...
     */
    async pushFilesToRepository(files, commitMessage) {
