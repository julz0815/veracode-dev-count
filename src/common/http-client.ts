...
import { GlobalNetworkConfig } from './types';
...
export class HttpClient {
   ...
   */
  async initialize(config: GlobalNetworkConfig): Promise<void> {
    ...
    if (process.argv.includes('--debug')) {
      console.log('HTTP Client initialized:', {
        ...
        agentRejectUnauthorized: this.httpsAgent ? (this.httpsAgent as any).options?.rejectUnauthorized : undefined,
   ...
   */
  private async createHttpsAgent(): Promise<https.Agent | undefined> {
    if (!this.sslConfig) {
      return undefined;
    }
  ...
  private async fetchViaProxy(url: string, options: RequestInit = {}): Promise<Response> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (proxy.protocol === 'https') {
        ...
        // Apply SSL config
        if (this.httpsAgent) {
          const agentOptions = (this.httpsAgent as any).options;
          if (agentOptions) {
        ...
        proxyTlsSocket.on('data', (chunk: Buffer) => {
          ...
          if (headerEnd !== -1) {
            ...
            if (statusMatch) {
              ...
              const targetTlsOptions: tls.ConnectionOptions = {
   ...
   */
  private async fetchHttpViaProxy(url: string, options: RequestInit = {}): Promise<Response> {
    const urlObj = new URL(url);
    const proxy = this.proxyConfig!;
    const proxyProtocol = proxy.protocol === 'https' ? https : http;
   ...
   */
  private async fetchWithHttpsAgent(url: string, options: RequestInit = {}): Promise<Response> {
    const urlObj = new URL(url);
